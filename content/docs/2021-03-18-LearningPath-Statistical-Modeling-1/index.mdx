...
### Geometric
Suppose we have a sequence of independent trials with the probability of success $p$ in each trial, then the probability distribution of a random variable $G$, where its outcomes are the possible number of trials required before first success appear, is called the geometric distribution.
