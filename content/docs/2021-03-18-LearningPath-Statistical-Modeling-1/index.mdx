...
where $0 < p < 1$ and $k = r,r+1,...$. In here, $k$ is the number of failures and $r$ is the number of successes.
...
At the end, we get a distribution of sums of squares of $k$ i.i.d random variables ($k$ samples from the standard normal distribution).
...
### F-distribution 
The F-distribution can be seen as a distribution of the ratio of two chi-square distributions, which is the ratio of two sums of squares of deviations of samples from their means therefore variance between samples and variance within samples are needed. A common definition is as follows:  
...
$\lambda e^{-\lambda x}$ is referred to as the exponential decay and is commonly used for measuring radioactive decay, medication within the human body, and population of endangered species. The probability density function of the exponential distribution gives the probability of decay at time $t$.  
