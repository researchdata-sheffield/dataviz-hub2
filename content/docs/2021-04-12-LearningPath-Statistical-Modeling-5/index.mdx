...
The CLT tells us that for any number of independent, identically distributed random variables with finite mean and variance (regardless what distribution it has!), its sum tends to be normally distributed as the number of experiments or sample size increases. The nature of the normal distribution implies the mean of a large number of random variables also approximate the normal distribution. This result usually requires the sample size to be greater than or equal to 30.  
...
See the following points for a simple explaination of each graph:
...
In part 6 we will be exploring the relationships between mean and variance for different distributions, solutions for data not satisfies the homogeneity of variance, and more on residuals.
