...
Much of modern research is interested in the relationship between variables. A statistical model is a formal representation of this relationship which lets us test a relationship, or predict the value of an unknown based on the value of a known quantity.   

The purpose for which the model is created and the data used will have a large influence on the way the model is interpreted. Experimental scientists may create models which assume a causal relationship between the variables, for example, rate of plant growth based on the application of fertilises. Others may be interested in the relationship between variables; how one may vary given changes in the other. For example, a meteorologist may like to predict how much precipitation a region may get, based on atmospheric pressure. 

There are numerous statistical models, they can be as simple as fitting a regression line between two continuous variables or as complex as a weather forecast, which require a high specification supercomputer to run. In all cases, models attempt to capture changes in a variable, based on known relationships. No model can do this perfectly, a statistical model is just an assumption. In all instances there will be unexplainable variability, or ‘noise’ accompanying our deterministic models. David Spiegelhalter describes models as maps of terrain. Not an image of the terrain. An important distinction, too often models are not shown the true amount of scepticism they deserve. With this in mind, let’s take a look at some powerful, yet simple statistical models that are used frequently in modern research. 
 
## Prerequisites
...
## Regression
Linear regression, as the name suggests, assumes a linear relationship between one or more independent variables and a single dependent variable. Linear regression is useful and very popular statistical modelling tool. 
...
The three most common linear regression models that you’re likely to come across are: Simple Linear regression, multiple linear regression, and Logistic regression.

### Simple linear regression
Simple linear regression is used on occasions with just two continuous variables. A straight line is fit thought the data which represents the linear relationship between the two variables. 
There are a number of ways estimate this linear relationship, by far the most common method is ordinary least squares. This method seeks to fit a line which minimises the sum of the squared residuals. 
A residual is the vertical distance from the observed value to the regression line. That may sound a little alien, it’s easiest to understand when it’s presented graphically. So let’s jump into a case study. 
...
Figure X, shows a scatter plot of crown volume against basal growth with a fitted regression line. The residuals are shown as vertical red lines. 
...
![Linear Regression](./Images/Trees2.png)
    *Figure 1 - Change in basal width and crown volume with a fitted least squares regression line.*

#### What does this model tell us?
So we have a fitted model. But what does it tell us? In Figure X I’ve shown you a fitted regression line on graph, many with be familiar with this. 
However, as we know, a model is a mathematical representation of the relationship between the two variables. This line can in fact be summarised by two coefficients. 
The slope of the line, and the value on the y axis where the line intercepts. Table X shows a typical output from a linear regression model. 
...
*Table 1 - Output from a simple linear regression model*
...
Shown in Table 1. In our example, this is the change in crown volume of our trees, depending on the change in basal growth. You may see the gradient of the regression line referred to as the regression coefficient. 
...
You can see from Figure 1 that the points are scattered around the regression line. The model lets us estimate the change in crown volume based on the change in basal growth, it does not provide the exact value. 
...
These assumptions are listed in a useful handbook, for commonly used statistical tests which can be found [here.](https://www.statstutor.ac.uk/resources/uploaded/tutorsquickguidetostatistics.pdf) 
...
For example, data collected over time may have an underlying seasonal component. We can test independence of observations with a Durbin Watson test. 

#### *Residuals should be normally distributed*

The residuals of our model are normally distributed. This is an interesting one, of you will find it stated that the data should be normally distributed. 
This is not the case. It is the residuals of the model that should be normally distributed. The residual values are the observed values minus the predicted values from them model. 
...
*Figure 3 - A quantile-quantile plot of residuals*

#### *A linear relationship between the dependent variable and the independent variable*

This is fairly self-explanatory, you can investigate this simply with a scatter plot. You are likely to be familiar with the cigar shape of variables that have a linear relationship. 

#### *Homoscedasticy* 

Now this is a complex work for a simple assumption. Homoscedasticity means ‘equal scatter’. In terms of our model, this means that our residuals should have no systematic change. 
We can check this by plotting our residuals against our predicted values. A strong pattern would be a cause for concern. 

#### *No outlying variables that have substantial influence on the fit*

Outlying variables can have a big impact on our linear models. To check that there are no outlying variables which are influencing the position and slope of our least squares line, we can look at Cook’s and leverage distances for each point.
 


## Multiple linear regression 

As you might imagine, multiple linear regression is used in those instances where there is more than independent variable. 
The principles are remarkably similar to that of simple linear regression, however we just have more information! We can use a multiple linear regression to improve our models by investigating how multiple independent variables influence our dependent variable. 

A classic example of multiple regression is how different factors affect the birthweight of a baby. 
...
The relationship isn’t as strong between birthweight and the height of the mother. It may still be a useful piece of information. 
When we run our linear model to include the second independent variable, the same principles apply. 
The least squares models finds the best mathematical representation of the relationship by fitting a plane to our data, and attempting to minimise the error. 
...
*Table 2 - Output from Multiple linear regression*

The interpretation of the results are a little different. Once again, we have an intercept, which tells us where our plane intersects the y-axis. 
...
In our example our model tells us that for every additional week of gestation there is a 0.151kg increase in birthweight. For every additional centimetre in the mother’s height, an additional 0.021kg increase in birthweight can be expected. 
...
For example, in our first instance, the value tells us whether the complex model, using both Gestation length and Mother’s height, is more effective than a model that just uses mother’s height. 
...
However, our independent variables can be expanded to include categorical variables, however, they do require some special preparation. 
...
All of the assumptions that are made for simple linear regression apply to multiple linear regression.
With the additional assumption that none of the independent variables in the model should be highly related. This can be tested very easily by checking that none are highly correlated. 
 
## Logistic regression
...
Why can’t we just use linear regression? There are a number of reasons why linear regression isn’t appropriate for a binary dependant variable. 
...
*Figure 4 - Engine shape and car weight fith a fitted logistic regression line.*
...
As we mentioned above the curve represents the probability that car has straight shaped engine based on the weight of the car. 
...
*Table 3 - Typical output from logistic regression*
...
In the case of our example, the odds of having a straight engine, over the odds of having a VS engine. 
...
The intercept is the value on the y axis when the weight of the vehicle is 0. So if we have a vehicle that weights nothing, the log(odds of a straight shaped engine) is 5.7. 
