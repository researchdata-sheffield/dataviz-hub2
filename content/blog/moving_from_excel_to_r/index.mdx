...
more strctured and detailed introduction to R, see following resources:
...
**Help**: If you are strugging with something, for example the meaning of `~`, type `help('~')` 
in the script and run and there will be relevant documentation appear in the *help* tab.
...
In R (and just like any other programming langauges) you can define your own functions, the 
...
```
Parameters or arguments are optional and could have an default value (like parameter2), if 
...
Note that this is for demostration and you can merge the function body into one line.
...
As you can see we have a dollar sign `$` in between the name of dataset and the name of particular column. This dollar sign allows us to extract 
...
## Data manipulation 
**[Tidyverse](https://www.tidyverse.org)** has a powerful package called *dplyr* designed for data manipulation and it works great with another package 
...
RStudio have prepared many useful [cheatsheets](https://rstudio.com/resources/cheatsheets/) for the *Tidyverse* package, I recommend you download them 
...
### group_by
The `group_by` function is particular useful when you need to aggregate data into some groups and then apply other manipulations.
...
### select 
If you want a subset of original data, `select()` method come in handy. To select columns, simply do:
```r
...
### slice 
This function is useful for select, remove and duplicate rows.
```r
...
When you using *ggplot2* all plots must begin with a call to the function `ggplot()`, within the 
...
of arguments that allows us to customise styles:
```r
...
are not well spreaded so I have installed a new package **ggrepel**:
...
Now the labels looks much more clear. We could also add a regression line to the plot:
```r {3}
ggplot(mtcars, aes(mpg, hp, label=rownames(mtcars))) + 
  geom_point(aes(color=factor(cyl)), size=4) +
...
Once you are happy with the layout, you might starting thinking about the theme 
...
This section will give some examples R codes for common functions in Excel. Note that these 
