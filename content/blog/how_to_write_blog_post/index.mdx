...
2. Create a file `index.mdx` (or with .md extension, but we recommend use .mdx because it’s a superset of Markdown syntax that also supports importing, exporting, and JSX. <a href="https://mdxjs.com/">learn more about mdx</a>). 
  ...
  2.1. At the very beginning of your index file, you have the following parameters to specify:
  ...
  2.2. Wrap parameters with triple hyphens. Use double quotation marks for all text, use square brackets in parameters with two or more items.

  2.3. Then add content to the file.

  2.4. (Optional): For the <i>first time writer</i> you're required to add your name, email and avatar to the file **author.json** located at `dataviz-hub2/src/author`, otherwise author info will not be shown on the post (and the website produce errors).
    Format: 
