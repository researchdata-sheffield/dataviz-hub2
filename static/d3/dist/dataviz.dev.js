...
function createMap(world, names, population) {
  ...
  svg.selectAll("path").data(countries).enter().append("path").attr("stroke", "#313131").attr("stroke-width", 1).attr("fill", "#535353").attr("d", path).on("mouseover", function (d, i) {
    ...
    tooltip.classed("hiddenTt", false).style("top", d3.event.pageY + -460 + "px").style("left", d3.event.pageX + 10 + "px").html(d.name + "<br/>" + "<p style='font-size: 13px'>Population: " + d.population + "</p><p style='font-size: 13px; margin-top: -10px'>Yearly change: " + d.Yearly_Change + "</p>");
