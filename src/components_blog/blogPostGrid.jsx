...
const blogPostGrid = ({allMdx}) => {
  ...
  return(
    ...
    {allMdx.edges.map(({ node }) => {
      ...
      return (
        <div key={node.id} className="w-full sm:w-1/2 md:w-1/2 lg:w-1/3 xl:w-1/3 min-h-100 pb-24" style={{fontFamily: "TUoS  Blake"}}>
          <Link className="group" to={node.fields.slug}>
            <img className="w-full blog-image" style={{minHeight: "60%", maxHeight: "60%",  objectFit: "cover", objectPosition: "center"}} src={imagesrc}></img>
