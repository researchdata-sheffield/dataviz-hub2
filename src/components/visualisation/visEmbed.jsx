 ...
 *  - custom: <boolean> whether the author passed custom embed code
 *  - url: <string> external url of the visualisation to be embed
 *  - height: <string> height of the visualisation in px
...
const visEmbed = (props) => {
  ...
  useEffect(() => {
    ...
    let element = document?.querySelector('#embedMenu')?.querySelector('.gatsby-highlight > pre > div');
  ...
  return (
    <div className="text-7xl" title="Embed visualisation">
      ...
      >
        ...
        {/* cancel button */}
        <button onClick={() => setEmbedMenu(!embedMenu)} className="text-gray-500 hover:text-gray-900 rounded-md transition duration-300 text-3xl p-1 absolute top-0 right-0 flex items-center" title="Close embed menu">
          <MdCancel />
        </button>
      </div>
    </div>
 ...
 * @returns 
 */
export function getCodeTemplate(mdx, type = "image", url = "", localPath = "", height = "550px") {
