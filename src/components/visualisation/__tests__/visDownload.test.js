...
describe("Create link for image to download", () => {
  it('should create the link and been clicked', () => {
    ...
    utils.createLinkForImage("downloadImage.jpg", "https://dataviz.shef.ac.uk/image.jpg");
...
describe("download the HTML element as png image", () => {
  ...
  it('should open the link if an external path provided', () => {
    utils.downloadAsPng(visualisationEl, mdxObj, "https://dataviz.shef.ac.uk/image.png")
    expect(window.open).toHaveBeenCalledTimes(1);
    expect(window.open).toHaveBeenCalledWith('https://dataviz.shef.ac.uk/image.png', '_blank', 'noopener,noreferrer');
  })

  it('should open the link if an internal path provided', () => {
    utils.downloadAsPng(visualisationEl, mdxObj, "2021-06-24-Increased-snack-intake-in-UK.png")
    expect(window.open).toHaveBeenCalledTimes(1);
    expect(window.open).toHaveBeenCalledWith('https://testURL.com/visualisation/2021-06-24-Increased-snack-intake-in-UK.png', '_blank', 'noopener,noreferrer');
  })

  it('should call createLinkForImage method if path not provided', () => {
    const spyToPng = jest.spyOn(htmlToImage, 'toPng');
    spyToPng.mockImplementation(() => Promise.resolve(utils.createLinkForImage()));
...
describe("handle image download for visualisation", () => {
  ...
  it('should use the PNG download method', () => {
    const spyToPng = jest.spyOn(utils, "downloadAsPng").mockImplementation(() => jest.fn());
  ...
  it('should use the SVG download method', () => {
    const spyToSvg = jest.spyOn(utils, "downloadAsSvg").mockImplementation(() => jest.fn());
