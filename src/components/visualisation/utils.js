...
export const downloadAsSvg = (visElement, mdxObject, svgPath = '') => {
  ...
  .then((dataURL) => {
    var image = new Image();
    image.src = dataURL;
    return createLinkForImage(`${kebabCase(mdxObject.frontmatter.title)}.svg`, dataURL);
  })
  .catch((error) => {
...
export const downloadAsPng = (visElement, mdxObject, pngPath = '') => {
  ...
  .then((dataURL) => {
    return createLinkForImage(`${kebabCase(mdxObject.frontmatter.title)}.png`, dataURL);
  })
  .catch((error) => {
    window.alert('Oops, something went wrong!', error.toString());
  });
