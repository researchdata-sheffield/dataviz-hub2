        ...
        {item.edges.map(({ node }) => {
          ...
          return (
            <Fade key={node.id} duration={2000} fraction={0.3}>
              <div key={node.id} className="group w-full sm:w-1/2 min-h-80 lg:min-h-95 xl:min-h-100 pb-28 overflow-hidden relative leading-none bg-gray-900 text-gray-300 flex-grow" tabIndex="0">
                <Link to={node.fields.slug}>
                  <div className="flex top-0 left-0 absolute m-3 lg:m-5 text-white lg:bg-transparent lg:text-gray-300 group-hover:bg-gray-900 items-center rounded-lg p-1" style={{zIndex: "3"}}>
                  ...
                  <IMG_DIV className="lg:greyScale-100 group-hover:greyScale-0 group-focus:greyScale-0 min-h-3/5 max-h-3/5" style={{minHeight: '420px'}}>
                  ...
                  <ArrowBox_featured className="pt-6 px-8 text-gray-600">
                    ...
                    <p className="my-2 text-xs 2xl:text-base pt-2 group-hover:text-gray-100">
                      {node.frontmatter.author.map((author, idx) => {
                        return (
                          <span key={node.id + '_author'}>{(node.frontmatter.author.length == idx + 1) ? author.name : author.name + " · "}</span>     
                        )
                      })}
                    </p>
